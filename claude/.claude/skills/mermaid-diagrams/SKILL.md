...
description: Creates diagrams in Mermaid.js syntax and should trigger when users ask for flowcharts, sequence diagrams, architecture diagrams, state diagrams, or other visual graphs that can be rendered with Mermaid CLI.
---
...
- Renders the diagram with Mermaid CLI so the result exists as an image file.
...
3. Save the Mermaid source to a `.mmd` file. Unless the user asked for a repo file, write render artifacts to a temporary or clearly disposable location such as `/tmp/mermaid-diagrams/`.
4. Render the diagram with Mermaid CLI using `mmdc`. Prefer `png` unless the user asked for `svg` or needs a scalable artifact.
5. Read the rendered image file and report the source and output paths.
